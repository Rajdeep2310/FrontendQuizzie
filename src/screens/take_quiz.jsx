...
const TakeQuize = () => {
  ...
  const fetchQ = async () => {
    setLoading(true)
    try {
      const { data } = await axios.get(`https://backendquizzie.onrender.com/quiz/take-quiz/${id}`)
      console.log(data)
  ...
  const submitHandler = async () => {
    setLoading(true);
    try {
      const { data } = await axios.post('https://backendquizzie.onrender.com/quiz/save-quiz-result', {
        quizeId: id,
