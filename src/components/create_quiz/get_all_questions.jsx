...
const GetAllQuestions = ({ setUrl, changePopup, quizeData, type }) => {
  ...
  const submitCreateQuize = async () => {
    try {
      const { data } = await axios.post('https://backendquizzie.onrender.com/quiz/create-quiz', quizeData, {
        headers: {
  ...
  const submitEditQuize = async () => {
    try {
      const { data } = await axios.put(`https://backendquizzie.onrender.com/quiz/${quizeData._id}`, quizeData, {
        headers: {
          Authorization: localStorage.getItem('authToken')
        }
      })
